...
```
<NAME in UPPERCASE block letters using █ ╗ ╔ ═ ║ ╚ ╝ chars, 6 lines tall>
<title> · <location>
Claude calls you <archetype>                         ← dim prefix + purple italic value
──────────────────────────────────────────────────────────
...
2. Render name in UPPERCASE using block letters made of `█ ╔ ╗ ║ ╚ ╝ ═` characters. If name is long, split first/last onto separate blocks
3. Title and location on one line, joined with ` · `
4. Archetype: "Claude calls you" prefix (dim) followed by archetype value (purple italic)
