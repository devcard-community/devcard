...
// --- ANSI Shadow Font (A-Z, 0-9, space) ---
...
function renderCard(data) {
  ...
  // ASCII Art Name
  const nameLines = renderName(data.name || 'DEV');
  out.push('');
  for (const line of nameLines) {
    out.push(`${c.claudeOrange}${line}${c.reset}`);
  }
  ...
  if (data.archetype) {
    out.push(`${c.gray}Claude calls you:${c.reset} ${c.brightMagenta}${c.italic}${data.archetype}${c.reset}`);
  }
