...
describe('archetype header consistency', () => {
  it('render-card.mjs shows archetype in header with "Claude calls you"', () => {
    const data = loadFixture('full-card.yaml');
    const output = renderCard(data);
    assert.ok(output.includes('Claude calls you'), 'Should contain "Claude calls you" prefix');
    assert.ok(output.includes('The Architect'), 'Should contain archetype name');
  });

  it('export-card.mjs shows archetype in header with "Claude calls you"', () => {
    const data = loadFixture('full-card.yaml');
    const svg = generateSVG(data);
    assert.ok(svg.includes('Claude calls you'), 'SVG should contain "Claude calls you"');
    assert.ok(svg.includes('The Architect'), 'SVG should contain archetype name');
  });

  it('serve-card.mjs shows archetype in header with "Claude calls you"', () => {
    const data = loadFixture('full-card.yaml');
    const html = generateHTML(data);
    assert.ok(html.includes('Claude calls you'), 'HTML should contain "Claude calls you"');
